...
require_relative "system/streakist/container"

Streakist::Container.boot! :bugsnag
...
namespace :db do
  ...
  DB = Sequel.connect(Streakist::Container.settings.database_url)
...
namespace :assets do
  ...
  task :clobber do
    require "fileutils"
    FileUtils.rm_rf("#{Streakist::Container.config.root}/public/assets")
  end
end
